...
import {createLastRow} from "../../http/itemAPI";
import {Button, Form, Modal} from "react-bootstrap";

const CreateLastRow = ({show, onHide}) => {
    ...
    const addItem = async () => {
        try {
            await createLastRow({itemId, serialNumber, softwareName, linkText, link, softwareVersion}).then(data => alert(data))
            setSerialNumber('')
            setSoftwareName('')
            setLinkText('')
            setLink('')
