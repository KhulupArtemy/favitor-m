...
import {createFirstRow} from "../../http/itemAPI";
import {useParams} from "react-router-dom";

const CreateFirstRow = ({show, onHide}) => {
    ...
    const addItem = async () => {
        try {
            await createFirstRow({itemId, serialNumber, softwareName, linkText, link, softwareVersion}).then(data => alert(data))
            setSerialNumber('')
            setSoftwareName('')
            setLinkText('')
            setLink('')
